...
func TestParseFuncMethod(t *testing.T) {
	tests := []struct {
		...
		{
			name:    "带导入路径的args参数",
			content: `func:name="approveFor"; args=["name string", "codes github.com/donutnomad/gogen/internal/utils#EString"]`,
			expected: &FuncMethodInfo{
				Name:       "approveFor",
				Attributes: map[string]string{},
				Args: []FuncMethodArg{
					{Name: "name", Type: "string"},
					{Name: "codes", Type: "EString", ImportPath: "github.com/donutnomad/gogen/internal/utils"},
				},
			},
		},
		{
			name:    "完整功能测试",
			content: `func:name="approveFor"; module="ABC"; event="EVENT"; nest=true; args=["name string", "codes github.com/donutnomad/gogen/internal/utils#EString"]`,
			expected: &FuncMethodInfo{
				Name: "approveFor",
				Attributes: map[string]string{
					"module": "ABC",
