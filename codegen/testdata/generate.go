...
func _codegen_getInfo[T any](v T) (code int, httpCode int, grpcCode codes.Code, name string, ok bool) {
	val := any(v)
	if _codegen_equal(val, ErrNotFound) {
		return 70001, 404, codes.NotFound, "ErrNotFound", true
	}
	if _codegen_equal(val, ErrUserNotFound) {
		return 70001, 404, codes.NotFound, "ErrUserNotFound", true
	}
	if _codegen_equal(val, ErrResourceNotFound) {
		return 70001, 404, codes.NotFound, "ErrResourceNotFound", true
	}
	if _codegen_equal(val, ErrBadRequest) {
		return 70003, 400, codes.InvalidArgument, "ErrBadRequest", true
	}
	if _codegen_equal(val, ErrInvalidInput) {
		return 70003, 400, codes.InvalidArgument, "ErrInvalidInput", true
	}
	if _codegen_equal(val, CodeInternal) {
		return 70002, 500, codes.Internal, "CodeInternal", true
	}
	if _codegen_equal(val, CodeServerError) {
		return 70002, 500, codes.Internal, "CodeServerError", true
	}
