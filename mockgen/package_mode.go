...
func parseExportFile(importPath string, ifaces []string, exportFile string) (*model.Package, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("read export data %q: %v", exportFile, err)
	}
	...
	interfaces, err := extractInterfacesFromPackageTypes(tp, ifaces)
	if err != nil {
		return nil, err
	}
