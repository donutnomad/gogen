...
// SourceModeOptions contains options for source mode parsing.
type SourceModeOptions struct {
	// Source is the input Go source file.
	Source string
	// Imports is comma-separated name=path pairs of explicit imports.
	Imports string
	// AuxFiles is comma-separated pkg=path pairs of auxiliary Go source files.
	AuxFiles string
	// ExcludeInterfaces is comma-separated names of interfaces to be excluded.
	ExcludeInterfaces string
	// IncludeInterfaces is a list of interface names to include.
	// If empty, all interfaces are included.
	IncludeInterfaces []string
}

// sourceMode generates mocks via source file.
func sourceMode(source string) (*model.Package, error) {
	return sourceModeWithOptions(SourceModeOptions{Source: source})
}

// sourceModeWithOptions generates mocks via source file with options.
func sourceModeWithOptions(opts SourceModeOptions) (*model.Package, error) {
	srcDir, err := filepath.Abs(filepath.Dir(opts.Source))
	if err != nil {
	...
	fs := token.NewFileSet()
	file, err := parser.ParseFile(fs, opts.Source, nil, 0)
	if err != nil {
	...
	// interface names -> include set
	if len(opts.IncludeInterfaces) > 0 {
		p.includeNamesSet = make(map[string]struct{}, len(opts.IncludeInterfaces))
		for _, name := range opts.IncludeInterfaces {
	...
	if opts.Imports != "" {
		for _, kv := range strings.Split(opts.Imports, ",") {
			eq := strings.Index(kv, "=")
			k, v := kv[:eq], kv[eq+1:]
			if k == "." {
				dotImports[v] = true
	...
	if opts.ExcludeInterfaces != "" {
		p.excludeNamesSet = parseExcludeInterfaces(opts.ExcludeInterfaces)
	}

	// Handle aux_files.
	if err := p.parseAuxFiles(opts.AuxFiles); err != nil {
		return nil, err
	}
